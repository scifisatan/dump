...
export const pingSchema = z.object({
  ok: z.boolean(), items: z.number(), version: z.number(), mode: z.enum(['local', 'cloud']),
});
...
export type Api = Hono<Record<string, never>, {
  '/api/ping': { $get: { input: Record<string, never>; output: PingResult; outputFormat: 'json'; status: 200 } };
}>;
