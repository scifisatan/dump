...
export const DEFAULT_LISTS: Collection[] = LISTS.map((list, position) => ({
  id: list.id, label: list.label, color: list.color, position, deleted: false,
}));
export const encodeCollection = (list: Collection): string => JSON.stringify(collectionSchema.parse(list));
export const decodeCollection = (data: unknown): Collection => collectionSchema.parse(JSON.parse(z.string().parse(data)));
...
export const dumpSchema = z.object({
  ...
  url: z.url().refine((url) => /^https?:\/\//i.test(url)).nullable(),
  done: z.boolean(),
  deleted: z.boolean(),
...
export const storeSchema = { dumps: { data: { type: 'string' as const } }, lists: { data: { type: 'string' as const } } };
export const encodeDump = (dump: Dump): string => JSON.stringify(dumpSchema.parse(dump));
export const decodeDump = (data: unknown): Dump => dumpSchema.parse(JSON.parse(z.string().parse(data)));
...
export function makeDump(raw: string, id = crypto.randomUUID(), now = Date.now(), lists: Collection[] = DEFAULT_LISTS): Dump {
  let text = raw.trim();
  const prefix = text.match(/^!([a-z0-9-]+)(?:\s+|$)/i);
  const name = prefix?.[1].toLowerCase();
  const list = lists.find((item) => !item.deleted && (item.id === name || item.label.toLowerCase().replace(/\s+/g, '-') === name))?.id ?? null;
  if (list) text = text.slice(prefix![0].length).trim();
  const tags = [...new Set([...text.matchAll(/(?:^|\s)#([\p{L}\p{N}_-]{1,64})(?=\s|$|[,.!?])/gu)]
    .map((match) => match[1].toLowerCase()))].slice(0, 30);
  const candidate = text.match(/https?:\/\/[^\s<>]+/i)?.[0].replace(/[),.!?;]+$/, '');
  let url: string | null = null;
  if (candidate) { try { url = new URL(candidate).href; } catch { /* Plain text remains intact. */ } }
  return dumpSchema.parse({
    id, text, list, tags, url, done: false, deleted: false,
    created_at: now, updated_at: now, classified_by: list ? 'user' : null,
  });
}
