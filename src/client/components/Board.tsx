  ...
  return (
    ...
    >
      <header className="column-header">
        ...
        <h2>{list.label}</h2>
        <span className="count">{dumps.length}</span>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" aria-label={`Options for ${list.label}`}>
      ...
      <div className="column-items">
        ...
        {!dumps.length && (
          <div className="column-empty">
            ...
            <span>Move a thought here from its menu.</span>
          </div>
        )}
      </div>
    </section>
...
export function Board({
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={reorder}>
      <div className="board" aria-label="Lists board">
        <section className="board-column inbox-column" aria-label="Inbox column">
          <header className="column-header">
            <span className="list-dot" />
            <h2>Inbox</h2>
            <span className="count">{dumps.filter((dump) => !dump.list).length}</span>
          </header>
          <div className="column-items">
            {dumps
            ...
            {!dumps.some((dump) => !dump.list) && (
              <div className="column-empty">
                A clear inbox.<span>New thoughts arrive here.</span>
              </div>
            )}
          </div>
        </section>
        ...
        <button className="new-column" onClick={create}>
          <Plus size={17} />
          New list
        </button>
      </div>
